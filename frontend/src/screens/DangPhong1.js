...
export default class DangPhong1 extends Component {
  ...
  handleUpdateQuan = (itemQuan) => {
    const dataXaPhuongNew = this.state.dataXaPhuongOrigin.filter(
      (item) => item.parent_code === itemQuan.code
    );
    this.setState({
      codeQuan: itemQuan,
  ...
  handleButtonTiepTheo = async () => {
    ...
    if (this.state.choPhepSuDungViTri) {
      ...
      this.props.navigation.navigate("DangPhong2", {
        address: address,

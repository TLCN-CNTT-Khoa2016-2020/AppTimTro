...
import BaiDangServices from "../Services/BaiDangServices";
...
export default class BaiDang extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      renderwhendelete: true,
    };
  }
  baiDaDuyet = () => {
    return this.state.dataBaiDaDuyetIsLoading ? (
    ...
    return this.state.dataBaiChuaDuyetIsLoading ? (
      ...
      <FlatList
        ...
        renderItem={({ item, index }) => (
          <CardPostHorizontal
            item={item}
            _deletePostChuaDuyet={this._deletePostChuaDuyet}
            isApprove={false}
            index={index}
            onPress={() =>
              this.props.navigation.navigate("XemBaiDang", {
        ...
        keyExtractor={(item, index) => String(index)}
        onEndReached={
          this.props.isPostUnApprovedEnd
            ? console.log("No more for loading !")
  ...
  loadBaiDaDuyet = async () => {
    ...
    await this.props.getPostApproved(
      authToken,
      this.state.baiDaDuyetPage,
      userID,
  ...
  loadBaiChuaDuyet = async () => {
    ...
    let dataUserID = await AsyncStorage.getItem("userID");
    let userID = await JSON.parse(dataUserID);
    await this.props.getPostUnApproved(
      authToken,
      this.state.baiChuaDuyetPage,
      userID,
      this.navigateToLoginScreen
    );
    this.setState({
